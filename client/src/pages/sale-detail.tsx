...
export default function SaleDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const data = await sdk.sales.findOne(id!);
        setSale(data);
      } catch {
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {(sale.lines ?? []).map((item) => {
                    ...
                    if (item.boxes > 0)
                      qtyParts.push(
                        `${item.boxes} ${item.boxes === 1 ? t("box") : t("boxesCount")}`,
                      );
                    if (item.bottles > 0)
                      qtyParts.push(
                        `${item.bottles} ${item.bottles === 1 ? t("bottle") : t("bottlesCount")}`,
                      );
                ...
                <tbody className="divide-y divide-border">
                  {sale.payments.length === 0 && (
                    <tr>
                      <td
                        colSpan={3}
                        className="px-4 sm:px-6 py-8 text-center text-muted-foreground italic"
                      >
