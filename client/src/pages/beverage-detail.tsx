...
function movFullLabel(
  ...
  if (bt > 0 || parts.length === 0)
    parts.push(`${bt} ${t("bottles").toLowerCase()}`);
...
function movLabel(
  ...
  if (m.bottlesDelta && m.bottlesDelta !== 0) {
    parts.push(
      `${movFullLabel(m, beverage, t)} ${t("fullStock").toLowerCase()}`,
    );
  }
  if (m.emptyBoxesDelta || m.emptyBottlesDelta) {
    ...
    if (ebtd !== 0 || sub.length === 0)
      sub.push(`${Math.abs(ebtd)} ${t("bottles").toLowerCase()}`);
    const isPositive =
      (m.emptyBoxesDelta ?? 0) + (m.emptyBottlesDelta ?? 0) > 0;
    parts.push(
      `${isPositive ? "+" : "-"}${sub.join(" + ")} ${t("emptyStock").toLowerCase()}`,
    );
  }
  return parts.join("  ·  ");
}
...
function InventoryModal({
  ...
  async function handleAdjust(e: React.FormEvent) {
    ...
    const fullDelta =
      (isRemoveFull ? -1 : 1) * (fb * beverage.bottlesPerBox + fbt);
  ...
  const fullComputed =
    (isRemoveFull ? -1 : 1) *
    ((parseInt(fullBoxes, 10) || 0) * beverage.bottlesPerBox +
      (parseInt(fullBottles, 10) || 0));
  const swapReceiving =
    (parseInt(swapBoxes, 10) || 0) * beverage.bottlesPerBox +
    (parseInt(swapBottles, 10) || 0);
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            ...
            <div className="flex gap-4 text-xs text-muted-foreground">
              <span>
                {t("fullStock")}: {stockLabel(beverage, t)}
              </span>
              <span>
        ...
        <div className="flex gap-1 border-b border-border bg-muted/30 px-4 pt-2 shrink-0">
          {(["adjust", "swap", "history"] as const).map((tt) => (
            <button
              ...
              type="button"
              onClick={() => setTab(tt)}
              className={`rounded-t-lg px-4 py-2 text-sm font-medium transition-colors ${tab === tt ? "border border-b-0 border-border bg-card text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              {tt === "adjust"
                ? t("addAdjust")
                : tt === "swap"
              ...
              <fieldset className="rounded-lg border border-border p-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <input
                      value={fullBoxes}
                      onChange={(e) => setFullBoxes(e.target.value)}
                      type="number"
                      min={0}
                      className={ic}
                      placeholder="0"
                ...
                <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={isRemoveFull}
                    onChange={(e) => setIsRemoveFull(e.target.checked)}
                ...
                {fullComputed !== 0 && (
                  <p
                    className={`mt-2 text-sm font-medium ${fullComputed > 0 ? "text-success" : "text-destructive"}`}
                  >
                    {fullComputed > 0 ? "+" : ""}
                    {fullComputed} {t("bottles").toLowerCase()}{" "}
                    {t("fullStock").toLowerCase()}
                  </p>
                )}
              </fieldset>
              ...
              <fieldset className="rounded-lg border border-border p-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <input
                      value={emptyBoxes}
                      onChange={(e) => setEmptyBoxes(e.target.value)}
                      type="number"
                      min={0}
                      className={ic}
                      placeholder="0"
            ...
            <form onSubmit={handleSwap} className="space-y-4 px-5 py-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    value={swapBoxes}
                    onChange={(e) => setSwapBoxes(e.target.value)}
                    type="number"
                    min={0}
                    className={ic}
                    placeholder="0"
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">
                    {t("emptyBottlesToSwap")}
                  </label>
                  <input
                    value={swapBottles}
                    onChange={(e) => setSwapBottles(e.target.value)}
                    type="number"
                    min={0}
                    className={ic}
                    placeholder="0"
              ...
              {swapReceiving > 0 && (
                <div className="rounded-lg border border-border bg-card px-4 py-3">
                  <p className="text-sm text-muted-foreground">
                    {t("fullBottlesReceived")}
                  </p>
                  <p className="text-lg font-semibold text-success">
                    +{swapReceiving} {t("bottles").toLowerCase()}
                  </p>
                </div>
              )}
              <div className="flex justify-end gap-2 pt-2">
              ...
              {movLoading ? (
                ...
                <div className="space-y-2">
                  {filteredMovements.map((m) => (
                    <div
                      key={m.id}
                      className="rounded-lg border border-border bg-background px-3 py-2.5 space-y-1"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm font-medium">
                          {ALL_REASONS.find((r) => r.value === m.reason)
                            ?.label ?? m.reason}
                        </span>
...
function EditModal({
  ...
  const [sizeMl, setSizeMl] = useState(
    beverage.sizeMl != null ? String(beverage.sizeMl) : "",
  );
  const [bottlesPerBox, setBottlesPerBox] = useState(
    String(beverage.bottlesPerBox),
  );
  const [saving, setSaving] = useState(false);
...
export default function BeverageDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("beverageNotFound"));
    } finally {
      setLoading(false);
    }
  }, [id, t]);
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p
                className={`mt-1.5 text-xl font-semibold tabular-nums ${isLow ? "text-amber-600" : ""}`}
              >
                {stockLabel(beverage, t)}
              </p>
              {isLow && (
                <p className="mt-1 flex items-center gap-1 text-xs text-amber-600">
                  <TrendingDown className="h-3 w-3" /> {t("lowStock")}
                </p>
              )}
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {beverage.bottlesPerBox} {t("btlPerBox")}
              </p>
            </div>
            <div className="rounded-lg bg-muted p-2">
                ...
                {currentPrice ? (
                  <div className="text-right text-sm">
                    <span className="tabular-nums">
                      {formatMoneyCents(currentPrice.pricePerBoxCents)}/
                      {t("box")}
                    </span>
                    <span className="ml-3 text-muted-foreground tabular-nums">
                      {formatMoneyCents(currentPrice.pricePerBottleCents)}/
                      {t("btl")}
                    </span>
                  </div>
                ) : (
        ...
        <div className="mb-4 flex flex-wrap gap-3">
          ...
          <div className="space-y-1">
            ...
            <select
              value={histReason}
              onChange={(e) => setHistReason(e.target.value)}
              className="h-9 rounded-lg border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring/40"
            >
              <option value="">{t("allReasons")}</option>
              {[
                "PURCHASE",
          ...
          {(histFrom || histTo || histReason) && (
            <button
              ...
              className="self-end rounded-lg border border-border px-3 py-1.5 text-sm text-muted-foreground hover:bg-accent"
            >
              {t("clear")}
            </button>
          )}
