...
export default function PaymentAccountsPage() {
  ...
  async function toggleActive(account: PaymentAccount) {
    ...
    try {
      const updated = await sdk.paymentAccounts.update(account.id, {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: t("holderName"),
      render: (a: PaymentAccount) => (
        <span className="text-muted-foreground">{a.holderName ?? "·"}</span>
      ),
    },
    {
      key: "bank",
      header: t("bankName"),
      render: (a: PaymentAccount) => (
        <span className="text-muted-foreground">{a.bankName ?? "·"}</span>
      ),
    },
    {
      key: "active",
      ...
      render: (a: PaymentAccount) => (
        <button
          ...
          title={
            a.isActive
              ? (t("activeClickToDeactivate") as string)
              : (t("inactiveClickToActivate") as string)
          }
        >
          <span
            className={`absolute top-0.5 h-4 w-4 rounded-full bg-white transition-all ${a.isActive ? "left-4" : "left-0.5"}`}
          />
        </button>
      ),
    },
