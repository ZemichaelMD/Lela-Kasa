...
export default function OrdersPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await apiGet(`/api/v1/orders/shop?status=${activeTab}&pageSize=50`);
  ...
  async function handleReject(id: string) {
    try {
      await apiPost(`/api/v1/orders/${id}/reject`, { reason: rejectReason || undefined });
          ...
          {orders.map((order: any) => (
            <Card key={order.id} className="p-4 space-y-2">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="font-bold">{formatMoneyCents(order.subtotalCents)}</span>
              </div>
              ...
              {order.status === 'PENDING' && (
                <div className="flex gap-2 pt-1">
                  <PermissionGate permission="orders:confirm">
                    ...
                    >
                      <Check className="h-3.5 w-3.5" /> {confirming === order.id ? '...' : t('confirmOrder')}
                    </button>
                  </PermissionGate>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-xl bg-card p-6 shadow-lg space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setRejectModal(null)} className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-accent">{t('cancel')}</button>
              <button onClick={() => handleReject(rejectModal)} className="rounded-lg bg-destructive px-4 py-2 text-sm font-medium text-destructive-foreground hover:bg-destructive/90">{t('rejectOrder')}</button>
            </div>
          </div>
        </div>
      )}
...
function StatusBadge({ status, t }: { status: string; t: (k: any) => string }) {
  ...
  return <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${color}`}>{t(`${status.toLowerCase()}`)}</span>;
}