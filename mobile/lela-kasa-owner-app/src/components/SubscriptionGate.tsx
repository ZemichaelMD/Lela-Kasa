...
import { getLanguage, setLanguage, subscribeLanguage, Language } from '../lib/i18n';
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.langBar, { borderBottomColor: colors.border }]}>
        ...
        >
          ...
          <Text style={[styles.langButtonText, { color: colors.textSecondary }]}>
            {lang === 'en' ? 'አማ' : 'EN'}
          </Text>
        </TouchableOpacity>
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {support && (
          <View style={[styles.contactCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text style={[styles.contactTitle, { color: colors.textPrimary }]}>Contact Admin</Text>
            {support.phone ? (
              <TouchableOpacity style={styles.contactRow} onPress={() => Linking.openURL(`tel:${support.phone}`)}>
                <Ionicons name="call-outline" size={16} color={colors.primary} />
                <Text style={[styles.contactText, { color: colors.primary }]}>{support.phone}</Text>
              </TouchableOpacity>
            ) : null}
            {support.email ? (
              <TouchableOpacity style={styles.contactRow} onPress={() => Linking.openURL(`mailto:${support.email}`)}>
                <Ionicons name="mail-outline" size={16} color={colors.primary} />
                <Text style={[styles.contactText, { color: colors.primary }]}>{support.email}</Text>
              </TouchableOpacity>
            ) : null}
            {support.telegram ? (
              <View style={styles.contactRow}>
                <Ionicons name="paper-plane-outline" size={16} color={colors.textMuted} />
                <Text style={[styles.contactText, { color: colors.textMuted }]}>Telegram: {support.telegram}</Text>
              </View>
            ) : null}
            {support.whatsapp ? (
              <View style={styles.contactRow}>
                <Ionicons name="logo-whatsapp" size={16} color={colors.textMuted} />
                <Text style={[styles.contactText, { color: colors.textMuted }]}>WhatsApp: {support.whatsapp}</Text>
              </View>
            ) : null}
            {support.hours ? (
              <View style={styles.contactRow}>
                <Ionicons name="time-outline" size={16} color={colors.textMuted} />
                <Text style={[styles.contactSmall, { color: colors.textMuted }]}>{support.hours}</Text>
              </View>
            ) : null}
            {support.message ? (
              <Text style={[styles.contactSmall, { color: colors.textMuted, marginTop: spacing[2], fontStyle: 'italic' }]}>{support.message}</Text>
            ) : null}
          </View>
        )}
