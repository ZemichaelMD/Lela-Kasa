...
const KIND_OPTIONS = [
  { value: 'CASH', label: 'Cash', icon: 'cash-outline' as keyof typeof Ionicons.glyphMap },
  { value: 'BANK', label: 'Bank', icon: 'business-outline' as keyof typeof Ionicons.glyphMap },
  { value: 'MOBILE_MONEY', label: 'Mobile Money', icon: 'phone-portrait-outline' as keyof typeof Ionicons.glyphMap },
  { value: 'OTHER', label: 'Other', icon: 'wallet-outline' as keyof typeof Ionicons.glyphMap },
];
...
export default function PaymentAccountsScreen() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: QK.paymentAccounts() }); setShowAddEditModal(false); resetForm(); showToast(t('paymentAccountCreated'), 'success'); },
    onError: () => showToast(t('failedToCreate'), 'error'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, dto }: { id: string; dto: any }) => getSdk().paymentAccounts.update(id, dto),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: QK.paymentAccounts() }); setShowAddEditModal(false); resetForm(); showToast(t('paymentAccountUpdated'), 'success'); },
    onError: () => showToast(t('failedToUpdate'), 'error'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => getSdk().paymentAccounts.delete(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: QK.paymentAccounts() }); setShowDeleteDialog(false); setDeleteTarget(null); showToast(t('paymentAccountDeleted'), 'success'); },
    onError: () => showToast(t('failedToDelete'), 'error'),
  });

  const resetForm = () => {
    setFormName(''); setFormKind('CASH'); setFormHolderName('');
    setFormBankName(''); setFormAccountNumber(''); setFormNotes('');
    setFormIsActive(true); setEditingAccount(null); setShowKindPicker(false);
  };

  const openAdd = () => { resetForm(); setShowAddEditModal(true); };
  const openEdit = (item: any) => {
    setEditingAccount(item);
    setFormName(item.name); setFormKind(item.kind);
    setFormHolderName(item.holderName ?? ''); setFormBankName(item.bankName ?? '');
    setFormAccountNumber(item.accountNumber ?? ''); setFormNotes(item.notes ?? '');
    setFormIsActive(item.isActive); setShowKindPicker(false);
    setShowAddEditModal(true);
  };

  const handleSave = () => {
    if (!formName.trim()) { showToast(t('enterAccountName'), 'error'); return; }
    const dto = {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>{t('paymentAccountsTitle')}</Text>
          ...
          <View key={item.id} style={[styles.card, { backgroundColor: colors.surface }]}>
            ...
            {(item.holderName || item.bankName || item.accountNumber) && (
              <View style={[styles.cardDetails, { borderTopColor: colors.border }]}>
                {[item.holderName, item.bankName, item.accountNumber ? `****${item.accountNumber.slice(-4)}` : null].filter(Boolean).map((v, i) => (
                  <Text key={i} style={[styles.detailText, { color: colors.textSecondary }]}>{v}</Text>
                ))}
              </View>
            )}
      ...
      <ModalSheet
        ...
        title={editingAccount ? t('editPaymentAccount') : t('addPaymentAccount')}
        footer={
          <TouchableOpacity
      ...
      >
        <Field label={t('accountName')}>
          <View style={inputStyle}>
            <Ionicons name="wallet-outline" size={16} color={colors.textMuted} style={{ marginRight: spacing[2] }} />
            <TextInput
              style={[styles.inputText, { color: colors.textPrimary }]}
              value={formName} onChangeText={setFormName}
              placeholder={t('enterAccountName')} placeholderTextColor={colors.textMuted}
            />
          </View>
        </Field>
        ...
        <Field label={t('accountType')}>
          ...
          >
            <View style={styles.kindSelected}>
              <Ionicons name={selectedKind?.icon ?? 'wallet-outline'} size={16} color={colors.textMuted} style={{ marginRight: spacing[2] }} />
              <Text style={[styles.inputText, { color: colors.textPrimary, flex: 0 }]}>{selectedKind?.label ?? formKind}</Text>
            </View>
            <Ionicons name={showKindPicker ? 'chevron-up' : 'chevron-down'} size={16} color={colors.textMuted} />
          </TouchableOpacity>
          {showKindPicker && (
            <View style={[styles.kindDropdown, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              {KIND_OPTIONS.map((opt, index) => (
                <TouchableOpacity
                  key={opt.value}
                  style={[
                    styles.kindOption,
                    index < KIND_OPTIONS.length - 1 && { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border },
                    formKind === opt.value && { backgroundColor: colors.surfaceTinted },
                  ]}
                  onPress={() => { setFormKind(opt.value); setShowKindPicker(false); }}
                >
                  <Ionicons name={opt.icon} size={16} color={formKind === opt.value ? colors.primary : colors.textSecondary} style={{ marginRight: spacing[3] }} />
                  <Text style={[styles.kindOptionText, { color: formKind === opt.value ? colors.primary : colors.textPrimary }]}>{opt.label}</Text>
                  {formKind === opt.value && <Ionicons name="checkmark" size={16} color={colors.primary} />}
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <Field label={t('holderName')}>
          <View style={inputStyle}>
            <Ionicons name="person-outline" size={16} color={colors.textMuted} style={{ marginRight: spacing[2] }} />
            <TextInput style={[styles.inputText, { color: colors.textPrimary }]} value={formHolderName} onChangeText={setFormHolderName} placeholder={t('holderName')} placeholderTextColor={colors.textMuted} />
          </View>
        </Field>
        ...
        {(formKind === 'BANK') && (
          <Field label={t('bankName')}>
            <View style={inputStyle}>
              <Ionicons name="business-outline" size={16} color={colors.textMuted} style={{ marginRight: spacing[2] }} />
              <TextInput style={[styles.inputText, { color: colors.textPrimary }]} value={formBankName} onChangeText={setFormBankName} placeholder={t('bankName')} placeholderTextColor={colors.textMuted} />
            </View>
          </Field>
        )}
        ...
        {(formKind === 'BANK' || formKind === 'MOBILE_MONEY') && (
          <Field label={t('accountNumber')}>
            <View style={inputStyle}>
              <Ionicons name="card-outline" size={16} color={colors.textMuted} style={{ marginRight: spacing[2] }} />
              <TextInput style={[styles.inputText, { color: colors.textPrimary }]} value={formAccountNumber} onChangeText={setFormAccountNumber} placeholder={t('accountNumber')} placeholderTextColor={colors.textMuted} keyboardType="number-pad" />
            </View>
          </Field>
        )}
        ...
        <Field label={t('notes')}>
          <View style={[inputStyle, styles.textAreaRow]}>
            <TextInput
              ...
              placeholder={t('notes')} placeholderTextColor={colors.textMuted}
        ...
        {editingAccount && (
          <View style={[styles.switchRow, { backgroundColor: colors.surfaceMuted, borderColor: colors.border }]}>
            <View>
              ...
              <Text style={[styles.switchDesc, { color: colors.textSecondary }]}>Enable this payment account</Text>
            </View>
            <Switch
              value={formIsActive}
...
const styles = StyleSheet.create({
  ...
  cardDetails: { flexDirection: 'row', gap: spacing[4], paddingTop: spacing[3], marginTop: spacing[3], borderTopWidth: StyleSheet.hairlineWidth },
  detailText: { ...type.caption },
  cardActions: { flexDirection: 'row', gap: spacing[2], justifyContent: 'flex-end', marginTop: spacing[3] },
  ...
  kindDropdown: { borderWidth: 1, borderRadius: radius.lg, marginTop: spacing[1], overflow: 'hidden' },
  kindOption: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing[4], paddingVertical: spacing[3] },
  kindOptionText: { ...type.body, flex: 1 },
  textAreaRow: { height: 'auto', alignItems: 'flex-start', paddingVertical: spacing[3] },
  textArea: { ...type.body, flex: 1, minHeight: 72, textAlignVertical: 'top' },
  switchRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: spacing[4], borderRadius: radius.lg, borderWidth: 1, marginBottom: spacing[4] },
