...
export default function PriceTiersScreen() {
  ...
  const [priceEdits, setPriceEdits] = useState<
    Record<string, { box: string; bottle: string }>
  >({});
  ...
  const { data: tierPrices, isLoading: loadingPrices } = useQuery({
    ...
    queryFn: () =>
      selectedTier ? getSdk().priceTiers.getPrices(selectedTier.id) : [],
    enabled: !!selectedTier,
  });
  ...
  const savePricesMutation = useMutation({
    mutationFn: ({ tierId, prices }: { tierId: string; prices: any[] }) =>
  ...
  const handleSavePrices = () => {
    ...
    const prices = beverages.data
      .map((bev) => {
        ...
        const boxCents = edit
          ? parseInt(edit.box, 10) * 100
          : (existing?.pricePerBoxCents ?? 0);
        const bottleCents = edit
          ? parseInt(edit.bottle, 10) * 100
          : (existing?.pricePerBottleCents ?? 0);
        return {
          beverageId: bev.id,
          pricePerBoxCents: isNaN(boxCents) ? 0 : boxCents,
          pricePerBottleCents: isNaN(bottleCents) ? 0 : bottleCents,
        };
      })
  ...
  const getPriceEdit = (bevId: string, field: "box" | "bottle") => {
    ...
    if (existing)
      return String(
        (field === "box"
          ? existing.pricePerBoxCents
          : existing.pricePerBottleCents) / 100,
      );
    return "";
  };
  ...
  return (
    ...
    >
        ...
        >
          <View style={styles.toggleInfo}>
            ...
            <Text style={[styles.toggleDesc, { color: colors.textSecondary }]}>
              Make this the default price tier
            </Text>
          </View>
          <View
            style={[
          ...
          >
            <View
              style={[
                styles.switchThumb,
                { transform: [{ translateX: formIsDefault ? 20 : 2 }] },
              ]}
            />
          </View>
        </TouchableOpacity>
      ...
      >
        {selectedTier && (
          <View
            style={[styles.tierBadge, { backgroundColor: colors.primaryLight }]}
          >
            <Text style={[styles.tierBadgeName, { color: colors.primary }]}>
              {selectedTier.name}
            </Text>
            {selectedTier.kind && (
              <Text style={[styles.tierBadgeKind, { color: colors.primary }]}>
                {selectedTier.kind}
              </Text>
            )}
          </View>
          ...
          <View style={styles.pricesList}>
            {beverages?.data.map((bev, index) => (
              ...
              >
                <View style={styles.priceInfo}>
                  <Text
                    style={[styles.priceName, { color: colors.textPrimary }]}
                  >
                    {bev.name}
                  </Text>
                  {bev.brand && (
                    <Text
                      style={[styles.priceBrand, { color: colors.textMuted }]}
                    >
                      {bev.brand}
                    </Text>
                  )}
            ...
            <TouchableOpacity
              style={[
                styles.footerBtn,
                { backgroundColor: colors.primary },
                savePricesMutation.isPending && styles.btnDisabled,
              ]}
