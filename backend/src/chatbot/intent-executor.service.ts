...
export class IntentExecutorService {
  ...
  async resolveEntities(
    ...
    switch (intent) {
      case 'register_sale': {
        return this.resolveSaleEntities(ctx, params as unknown as RegisterSaleParams);
      }
      case 'register_payment': {
        return this.resolvePaymentEntities(ctx, params as unknown as RegisterPaymentParams);
      }
      case 'register_return': {
        return this.resolveReturnEntities(ctx, params as unknown as RegisterReturnParams);
      }
      case 'check_balance': {
        return this.resolveBalanceQuery(ctx, shopId, params as unknown as CheckBalanceParams);
      }
      case 'check_stock': {
        return this.resolveStockQuery(ctx, shopId, params as CheckStockParams);
      }
      case 'summary': {
        return this.resolveSummaryQuery(shopId, params as unknown as SummaryParams);
      }
      case 'help': {
        return {
      ...
      default: {
        return {
          summary: { customer: { id: '', name: '', matched: false } },
          needsClarification: "I'm not sure I understood. Type 'help' for examples.",
        };
      }
    }
  }

  private findCustomer(ctx: { customers: Array<{ id: string; name: string }> }, name: string): { customer: { id: string; name: string; matched: boolean } | null; clarification?: string; options?: string[] } {
    if (!name) return { customer: null, clarification: 'Please specify a customer name.' };

    const lower = name.toLowerCase();
    const exact = ctx.customers.find((c) => c.name.toLowerCase() === lower);
    if (exact) return { customer: { id: exact.id, name: exact.name, matched: true } };

    const contains = ctx.customers.filter((c) => c.name.toLowerCase().includes(lower));
    if (contains.length === 1) return { customer: { id: contains[0].id, name: contains[0].name, matched: true } };
    ...
    if (fuzzy.length === 1) return { customer: { id: fuzzy[0].id, name: fuzzy[0].name, matched: true } };
    if (fuzzy.length > 1) {
      return {
        customer: null,
  ...
  private findBeverage(
    ctx: { beverages: Array<{ id: string; name: string; brand: string | null; bottlesPerBox: number; stockBottles: number; pricePerBoxCents?: number; pricePerBottleCents?: number }> },
    name: string,
  ): { beverage: { id: string; name: string; brand: string | null; bottlesPerBox: number; stockBottles: number; matched: boolean } | null; clarification?: string; options?: string[] } {
    if (!name) return { beverage: null, clarification: 'Please specify a beverage name.' };

    const lower = name.toLowerCase();
    const exact = ctx.beverages.find((b) => b.name.toLowerCase() === lower || (b.brand && b.brand.toLowerCase() === lower));
    if (exact) return { beverage: { id: exact.id, name: exact.name, brand: exact.brand, bottlesPerBox: exact.bottlesPerBox, stockBottles: exact.stockBottles, matched: true } };

    const contains = ctx.beverages.filter((b) => b.name.toLowerCase().includes(lower) || (b.brand && b.brand.toLowerCase().includes(lower)));
    if (contains.length === 1) return { beverage: { id: contains[0].id, name: contains[0].name, brand: contains[0].brand, bottlesPerBox: contains[0].bottlesPerBox, stockBottles: contains[0].stockBottles, matched: true } };
    if (contains.length > 1) {
      return {
        beverage: null,
  ...
  private findPaymentAccount(
    ...
    name?: string,
    method?: string,
  ): { account: { id: string; name: string; kind: string; matched: boolean } | null; clarification?: string; options?: string[] } {
    if (!name) {
      // Default to first cash account
      const cashAccount = ctx.paymentAccounts.find((a) => a.kind === 'CASH_PERSON');
      if (cashAccount) return { account: { id: cashAccount.id, name: cashAccount.name, kind: cashAccount.kind, matched: true } };
      if (ctx.paymentAccounts.length > 0) return { account: { id: ctx.paymentAccounts[0].id, name: ctx.paymentAccounts[0].name, kind: ctx.paymentAccounts[0].kind, matched: true } };
      return { account: null, clarification: 'No payment accounts found. Please create one first.' };
    }

    const lower = name.toLowerCase();
    const exact = ctx.paymentAccounts.find((a) => a.name.toLowerCase() === lower);
    if (exact) return { account: { id: exact.id, name: exact.name, kind: exact.kind, matched: true } };

    const contains = ctx.paymentAccounts.filter((a) => a.name.toLowerCase().includes(lower));
    if (contains.length === 1) return { account: { id: contains[0].id, name: contains[0].name, kind: contains[0].kind, matched: true } };
    if (contains.length > 1) {
      return {
        account: null,
  ...
  private async resolveSaleEntities(
    ...
    if (!customerResult.customer) {
      return {
        summary: { customer: { id: '', name: params.customerName, matched: false } },
    ...
    const lines: Array<{ beverage: ResolvedBeverage; boxes: number; bottles: number; lineTotalCents: number }> = [];
    let subtotalCents = 0;
    ...
    for (const lineParam of params.lines) {
      ...
      const lineTotalCents = lineParam.boxes * pricePerBoxCents + lineParam.bottles * pricePerBottleCents;
    ...
    const payments: Array<{ amountCents: number; method: string; account: ResolvedPaymentAccount }> = [];
    let paidCents = 0;

    for (const payParam of params.payments) {
      const accResult = this.findPaymentAccount(ctx, payParam.accountName, payParam.method);
      ...
      payments.push({ amountCents, method: payParam.method, account: accResult.account });
    ...
    // Build summary text
    const linesText = lines.map((l) => `  ${l.boxes > 0 ? l.boxes + ' box(es) ' : ''}${l.bottles > 0 ? l.bottles + ' bottle(s) ' : ''}${l.beverage.name}`).join('\n');
    const paymentsText = payments.map((p) => `  ${formatBirr(p.amountCents)} ETB (${p.method})`).join('\n');
    const creditText = creditDeltaCents > 0 ? `\n  Credit: ${formatBirr(creditDeltaCents)} ETB` : '';
  ...
  private async resolvePaymentEntities(
    ...
    const accResult = this.findPaymentAccount(ctx, params.accountName, params.method);
    if (!accResult.account) {
      return {
    ...
    const customer = await this.customersService.findOne('', customerResult.customer.id);
    ...
    return {
      ...
      summary: {
        customer: customerResult.customer,
        payments: [{ amountCents, method: params.method, account: accResult.account }],
      },
      answer: summaryText,
    };
  }
  ...
  private async resolveBalanceQuery(
    ...
    if (params.showAllWithCredit) {
      const customers = await this.prisma.customer.findMany({
        ...
        select: { name: true, creditBalanceCents: true, outstandingBoxes: true, outstandingBottles: true },
      ...
      const lines = customers.map((c) => `  ${c.name}: ${formatBirr(c.creditBalanceCents)} ETB | ${c.outstandingBoxes} boxes | ${c.outstandingBottles} bottles`).join('\n');
      return {
        summary: { customer: { id: '', name: '', matched: false } },
    ...
    const customer = await this.customersService.findOne(shopId, customerResult.customer.id);
  ...
  private async resolveStockQuery(
    ...
    if (params.showLowStock) {
      const allStock = ctx.beverages.map((b) => {
        const boxes = Math.floor(b.stockBottles / b.bottlesPerBox);
        const loose = b.stockBottles % b.bottlesPerBox;
        const isLow = b.stockBottles < b.bottlesPerBox * ctx.lowStockThreshold;
    ...
    if (params.beverageName) {
      ...
      const boxes = Math.floor(bevResult.beverage.stockBottles / bevResult.beverage.bottlesPerBox);
      const loose = bevResult.beverage.stockBottles % bevResult.beverage.bottlesPerBox;
      const status = bevResult.beverage.stockBottles === 0 ? '🔴 Out of stock' : bevResult.beverage.stockBottles < bevResult.beverage.bottlesPerBox * 2 ? '⚠️ Low' : '✅';
    ...
    // Show all stock
    const lines = ctx.beverages.map((b) => {
      const boxes = Math.floor(b.stockBottles / b.bottlesPerBox);
      const loose = b.stockBottles % b.bottlesPerBox;
      const status = b.stockBottles === 0 ? '🔴' : b.stockBottles < b.bottlesPerBox * 2 ? '⚠️' : '✅';
  ...
  private async resolveSummaryQuery(shopId: string, params: SummaryParams) {
    ...
    const summary = await this.reportsService.salesSummary(shopId, { from: from.toISOString(), to: now.toISOString() });

    const periodLabel = params.period === 'today' ? "Today's" : params.period === 'week' ? "This Week's" : "This Month's";
...
Type your request in English or Amharic!`;
  ...
  async executeSale(
    ...
    userId: string,
    params: RegisterSaleParams,
    resolved: { customerId: string; beverageIds?: string[]; paymentAccountIds?: string[]; summary: ChatConfirmationSummary },
  ) {
    ...
    const createSaleDto = {
      ...
      lines: resolved.summary.lines?.map((l, i) => ({
        beverageId: l.beverage.id,
        boxes: l.boxes,
        bottles: l.bottles,
      })) || [],
      payments: resolved.summary.payments?.map((p, i) => ({
    ...
    const sale = await this.salesService.createSale(shopId, userId, createSaleDto);
    ...
    return { type: 'success', message: `✅ Sale registered: ${resolved.summary.customer.name} — ${resolved.summary.lines?.length || 0} item(s) — ${formatBirr(resolved.summary.paidCents || 0)} ETB paid` };
  }
  ...
  async executePayment(
    ...
    userId: string,
    params: RegisterPaymentParams,
    resolved: { customerId: string; paymentAccountIds?: string[]; summary: ChatConfirmationSummary },
  ) {
    ...
    return { type: 'success', message: `✅ Payment recorded: ${resolved.summary.customer.name} — ${formatBirr(payment.amountCents)} ETB` };
  }
  ...
  async executeReturn(
    ...
    return { type: 'success', message: `✅ Return recorded: ${resolved.summary.customer.name} — ${resolved.summary.boxes} box(es), ${resolved.summary.bottles} bottle(s)` };
  }
}
