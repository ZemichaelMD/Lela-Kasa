  ...
  if (!account) {
    return (
      <div className="space-y-4">
        ...
        <Card className="p-12 text-center text-muted-foreground">{t('paymentAccountNotFound')}</Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Card className="p-4">
          <p className="text-xs text-muted-foreground mb-1">{t('totalPayments')}</p>
          <p className="text-2xl font-bold tabular-nums">{formatMoneyCents(totalAmount)}</p>
          <p className="text-xs text-muted-foreground mt-2">{sortedPayments.length} {t('transactions')}</p>
        </Card>
      </div>
