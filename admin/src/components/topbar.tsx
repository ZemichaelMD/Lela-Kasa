...
function NotificationsBell() {
  ...
  const fetchCount = useCallback(async () => {
    try { const { count: c } = await sdk.admin.getPendingPaymentCount(); setCount(c); } catch {}
  }, []);

  useEffect(() => { void fetchCount(); const i = setInterval(fetchCount, 15000); return () => clearInterval(i); }, [fetchCount]);
  useEffect(() => {
    function f(e: MouseEvent) { if (ref.current && !ref.current.contains(e.target as Node)) setOpen(false); }
    document.addEventListener("mousedown", f); return () => document.removeEventListener("mousedown", f);
  }, []);

  async function handleOpen() {
    setOpen(v => !v);
    if (!open) { try { setPayments(await sdk.admin.listPendingPayments()); } catch {} }
  }

  async function handleVerify(id: string) {
    setVerifying(id);
    try { await sdk.admin.verifyPayment(id); toast.success("Payment verified"); setPayments(p => p.filter(x => x.id !== id)); setCount(c => Math.max(0, c - 1)); }
    catch { toast.error("Failed to verify"); } finally { setVerifying(null); }
  }
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 rounded-lg border border-border bg-popover shadow-md z-50 text-sm">
          <div className="px-3 py-2 text-xs font-semibold text-muted-foreground border-b border-border">Notifications</div>
          {payments.length === 0 ? (
            <div className="px-4 py-6 text-center text-xs text-muted-foreground">All caught up!</div>
          ) : (
            <div className="max-h-72 overflow-y-auto">
              {payments.map(p => (
                <div key={p.id} className="border-b border-border last:border-0 px-3 py-2.5 space-y-1.5">
                  <button type="button" onClick={() => window.location.href = `/shops/${p.shopId}?tab=subscription`} className="w-full text-left">
                    <div className="flex items-start justify-between gap-2">
                      <span className="font-medium text-xs">{p.shop?.name ?? "Shop"}</span>
                      <span className="shrink-0 text-[10px] font-semibold">{p.amountCents ? `${(p.amountCents / 100).toFixed(2)} ETB` : "—"}</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground">{p.provider?.name}{p.reference ? ` · ${p.reference}` : ""}</p>
                  </button>
                  <button type="button" onClick={() => handleVerify(p.id)} disabled={verifying === p.id}
...
function CustomerSearch() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { const data = await sdk.customers.list({ search: query.trim(), pageSize: 8 }); setResults(data.data); setOpen(true); }
      catch { /* ignore */ }
    }, 300);
  ...
  function selectCustomer(c: Customer) { setQuery(""); setResults([]); setOpen(false); navigate(`/customers/${c.id}`); }
  ...
  return (
    <div className="relative hidden md:block w-48 lg:w-64" ref={ref}>
      <div className="flex h-8 items-center gap-2 rounded-lg border border-border bg-muted/30 px-2.5 text-sm text-muted-foreground focus-within:border-primary/50 focus-within:bg-card">
        ...
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search..." className="flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground text-foreground" />
        {query && <button type="button" onClick={() => { setQuery(""); setResults([]); setOpen(false); }} className="rounded p-0.5 text-muted-foreground hover:text-foreground"><X className="h-3 w-3" /></button>}
      </div>
      ...
      {open && results.length === 0 && query.trim() && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 rounded-lg border border-border bg-popover px-3 py-3 text-center text-xs text-muted-foreground shadow-md">No matches</div>
      )}
    </div>
  );
}
...
export function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  useEffect(() => {
    function f(e: MouseEvent) { if (menuRef.current && !menuRef.current.contains(e.target as Node)) setMenuOpen(false); }
    document.addEventListener("mousedown", f); return () => document.removeEventListener("mousedown", f);
  }, []);
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div className="relative" ref={menuRef}>
          <button type="button" onClick={() => setMenuOpen(v => !v)}
            ...
            <span className="flex h-6 w-6 items-center justify-center rounded bg-primary text-[10px] font-bold text-primary-foreground">{initial}</span>
            <span className="hidden md:inline font-medium text-foreground">{user?.name ?? "Admin"}</span>
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-1.5 w-48 rounded-lg border border-border bg-popover p-1 shadow-md text-xs">
              <div className="px-2.5 py-2 text-muted-foreground border-b border-border">
                ...
                <p className="text-[10px] uppercase tracking-wider mt-0.5">{user?.role ?? "USER"}</p>
              </div>
