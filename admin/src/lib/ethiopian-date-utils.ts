...
function grigorianDateFromDayNo(dayNum: number): Date {
  ...
  const num400 = Math.floor(dayNum / 146097);
  dayNum %= 146097;
  if (dayNum === 0) {
    return new Date(400 * num400, 12 - 1, 31);
  }

  const num100 = Math.min(Math.floor(dayNum / 36524), 3);
  dayNum -= num100 * 36524;
  if (dayNum === 0) {
    return new Date(400 * num400 + 100 * num100, 12 - 1, 31);
  }

  const num4 = Math.floor(dayNum / 1461);
  dayNum %= 1461;
  if (dayNum === 0) {
    return new Date(400 * num400 + 100 * num100 + 4 * num4, 12 - 1, 31);
  }

  const num1 = Math.min(Math.floor(dayNum / 365), 3);
  dayNum -= num1 * 365;
  if (dayNum === 0) {
...
export function formatEthiopianDate(
  ...
  switch (formatStr) {
    ...
    case "yyyy-MM-dd":
      return `${etDate.Year}-${etDate.Month.toString().padStart(2, "0")}-${etDate.Day.toString().padStart(2, "0")}`;
    case "yyyy-MM":
